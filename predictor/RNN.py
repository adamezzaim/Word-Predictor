...
ALL_CHARS = [' ', '’', '—', '–', '“', '”', 'é', '‘'] + list(string.punctuation) + list(string.ascii_letters) + list(
    string.digits)
...
class NLM:
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                for i in range(1, n_predict):
                    x_emb = self.char_emb(torch.LongTensor([x_preds[i - 1]])).reshape((1, 1, self.n_char_emb)).to(
                        self.device)
    ...
    def create_datasets(self, new_train_filename, new_test_filename):
        ...
        with open(new_train_filename, 'w+', encoding='utf8') as train_file, open(new_test_filename, 'w+',
                                                                                 encoding='utf8') as test_file, open(
                self.filename, encoding='utf8', errors='ignore') as data_file:
            all_lines = []
...
if __name__ == '__main__':
    ...
    nlm = NLM(args.train_file, model_news=args.train_news, fake_test=True, learning_rate=args.learning_rate,
              model_lower=args.lower_case)
    if args.load_model:
        nlm.load_model()
    if args.train:
